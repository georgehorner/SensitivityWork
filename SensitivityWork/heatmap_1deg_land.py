...
for year in range(2000,2017,1):
    ...
    landocean = xr.open_mfdataset('/net/hardin/disk1/Users/gah20/TSC_2origin/'+str(year)+'/*.nc',combine='nested',concat_dim='time')['ConvOrigin']
    ...
    lats_d = np.where(landocean[:,2:62]>0,lats_d,np.nan)
    lons_d = np.where(landocean[:,2:62]>0,lons_d,np.nan)
    ...
    for i in range(0,60,1):
        for j in range(0,360,1):
            ...
            if j%10==0:
                print(str(j) + ' done')
        print(str(i) + ' done!')
    np.save('/net/hardin/disk1/Users/gah20/Heatmaps/heatmap_1deg_land'+str(year)+'.npy',heatmap_d)
    print(str(year) + ' done')
