...
for year in range(2014,2017,1):
    ...
    lats_d = np.where(landocean<0,lats_d,np.nan)
    lons_d = np.where(landocean<0,lons_d,np.nan)
    ...
    for i in range(0,60,1):
        for j in range(0,360,1):
            ...
            if j%10==0:
                print(str(j) + ' done')
        print(str(i) + ' done!')
    np.save('/net/hardin/disk1/Users/gah20/Heatmaps/heatmap_1deg_ocean'+str(year)+'.npy',heatmap_d)
    print(str(year) + ' done')
